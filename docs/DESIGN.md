...
| 10 | LLM and backend | Claude Opus 5 behind a FastAPI backend on Cloud Run from day one. Firebase Auth with an email allowlist. Only the backend URL ships in the APK. |
| 11 | Repo | Monorepo: `android/`, `backend/`, `contracts/`. Both sides test against the same JSON Schemas and examples. |
